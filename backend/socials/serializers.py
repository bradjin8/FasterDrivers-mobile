...
class CustomAppleSocialLoginSerializer(SocialLoginSerializer):
    ...
    type = serializers.ChoiceField(choices=USER_TYPES, required=False, allow_blank=True)
    ...
    def validate(self, attrs):
        ...
        if type:
            ...
            if not user.type:
                if attrs.get('type') == 'Customer':
                    Customer.objects.create(
                        user=user
                    )
                    user.type = 'Customer'
                elif attrs.get('type') == 'Driver':
                    Driver.objects.create(
                        user=user
                    )
                    user.type = 'Driver'
                elif attrs.get('type') == 'Restaurant':
            ...
            if not user.type:
                user.delete()
                raise serializers.ValidationError({'detail': 'You must register before attempting to login'})
        return attrs
